...
if (class_exists('GFForms')) {
	class GF_Appointment_Booking_Providers extends GF_Field
	{
		...
		const GLOBAL_PROVIDER = 'No preference';
		const GLOBAL_PROVIDER_ID = 0;
		...
		*/
		 ...
		 */
		public static function get_providers_choices( $form, $value, $new_service_id, &$provider_id = 0 )
		{
			$options = '';
			...
			if( $the_query->have_posts() ) {
				while( $the_query->have_posts() ) {
					$the_query->the_post();
                    $post = $the_query->post;
                    if( $value == $post->post_title ) {
                        $selected = ' selected="selected"';
                        $provider_id = $post->ID;
                    } else {
                        $selected = "";
                    }
					$options .= '<option value="' . esc_html($post->post_title) . '"' . $selected . '>' . $post->post_title . '</option>' . PHP_EOL;
                }

				wp_reset_postdata();
			} else {
				$options .= "<option value='" . self::GLOBAL_PROVIDER_ID . "'> " . self::GLOBAL_PROVIDER . "</option>" . PHP_EOL;
			}
		...
		public function validate($value, $form)
		{
            ...
            $provider_id = get_page_by_title( $value, OBJECT, 'ga_providers' );
        ...
        */
        ...
        public static function format_entry_field( $value ) {
            ...
            if( is_numeric( $value ) ) {
                ...
                if( 'ga_providers' == get_post_type( $post_id ) ) {
                    $value = get_the_title( $post_id );
                } else if( $post_id === self::GLOBAL_PROVIDER_ID ) {
                    $value = self::GLOBAL_PROVIDER;
                }
            }
