    ...
    function conwr_check_api_key($function_name = "") {
        ...
        if (isset($sc_email) && !empty($sc_email) && isset($sc_api_key) && !empty($sc_api_key)) {
            ...
            if (isset($response) && $response == "ok") {
                ...
                conwr_api_write_log("Could not verify email or API key, response is not valid. Email: $sc_email; API Key: $sc_api_key; Response: $response. Function: $function_name.");
            }
        }
