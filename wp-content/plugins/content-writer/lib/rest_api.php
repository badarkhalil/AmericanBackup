...
if (!function_exists('conwr_restapi_copy_post_image')) {
    ...
    function conwr_restapi_copy_post_image($image_url, $dest_file) {
        ...
        // Hardcoded url with different method
		$bypass_url = CONWR_PLUGIN_API_URL_WITHOUT_METHOD_NAME . '/GetPostImage?url='. $image_url;
		
		$output = wp_remote_get($bypass_url, $args);

		if(is_wp_error($output)) {
			conwr_api_write_log("Error while trying to copy image from url: $image_url. Reason: " . $output->get_error_message() . "; Function: " . __FUNCTION__);
			return false;
		}
		else {
			//save response as image in wp-content folder at provided path
			file_put_contents($dest_file, base64_decode($output['body']));
